...
Cách nhanh nhất để chạy project mà không cần cài JDK/Maven trên máy.
...
**Lưu ý lần đầu chạy:** Mất khoảng 5-8 phút để Docker pull base image, download Maven dependencies, và build JAR. Các lần sau nhanh hơn nhiều nhờ cache layer.

App sẽ chạy ở `http://localhost:8081`.

### Cách 2: Chạy local (cần cài Java + Maven)
...
### Các URL quan trọng (cho cả 2 cách)
...
> **Lưu ý:** H2 in-memory đồng nghĩa với việc data sẽ mất khi restart app/container. Đây là behavior phù hợp cho dev/test, không phù hợp production.
...
├── .dockerignore               # Loại file khỏi Docker build context
├── pom.xml
├── README.md
...
### 7.7. Multi-stage Docker build
Dockerfile chia 2 stage: `builder` (chứa Maven + JDK để build) và `runtime` (chỉ JRE + JAR). Image cuối cùng nhỏ hơn nhiều (~200MB so với ~600MB nếu dùng 1 stage), không có Maven/source code trong production image.
...
- **Persistent database** — có thể swap H2 sang PostgreSQL với một số thay đổi nhỏ trong `pom.xml` và `application.yml`
